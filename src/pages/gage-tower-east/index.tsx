import ATSInspection from "../../components/atsInspection";
import Checklist from "../../components/checklist";
import type { HeaderProps } from "../../components/header";
import Header from "../../components/header";
import type { ImagesProps } from "../../components/images";
import Images from "../../components/images";
import type { NotesProps } from "../../components/notes";
import Notes from "../../components/notes";
import Procedure from "../../components/procedure";

type GageTowerEastProps = {
  buildingInfo: HeaderProps;
  emergencyGeneratorInformation: ImagesProps;
  routeToFrontDesk: ImagesProps;
  routeToGenATSAndFP: ImagesProps;
  notes: NotesProps;
};

function GageTowerEast({
  buildingInfo,
  emergencyGeneratorInformation,
  notes,
  routeToFrontDesk,
  routeToGenATSAndFP,
}: GageTowerEastProps) {
  return (
    <div className="building">
      <Header {...buildingInfo} />
      <Images {...emergencyGeneratorInformation} />
      <Images {...routeToFrontDesk} />
      <Images {...routeToGenATSAndFP} />
      <Checklist />
      <ATSInspection atsKind="TSC 80e" />
      <Notes {...notes} />
      <Procedure />
    </div>
  );
}

export default GageTowerEast;
export type { GageTowerEastProps };
